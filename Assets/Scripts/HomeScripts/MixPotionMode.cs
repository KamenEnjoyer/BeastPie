...
public class MixPotionMode : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        leftIngredient.GetComponent<Button>().onClick.AddListener(() => { leftIngredient.Clear(false, true); });
        rightIngredient.GetComponent<Button>().onClick.AddListener(() => { rightIngredient.Clear(false, true); });
    ...
    private StorageContentData CreatePotion(StorageContentData a, StorageContentData b)
    {
        ...
        else
        {
            ...
            potion.data.id = GILData.IngredientType.Potion.ToString() + newPotionId;
